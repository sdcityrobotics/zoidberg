...
try:
    ...
    print('Move the accerometer for %.1f seconds'%np.max(taxis))
    xacc, yacc, zacc = read_accelerometer(mav, taxis)
finally:
...
ax.set_title('Recorded accelerometer data')
ax.plot(taxis, xacc, label='xacc')
ax.plot(taxis, yacc, label='yacc')
ax.plot(taxis, zacc, label='zacc')
ax.grid()
ax.legend()
ax.set_ylabel('acceleration, mGal')
